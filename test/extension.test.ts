...
function writeConfig(config: unknown): void {
  writeFileSync(join(agentDir, "sandbox.json"), JSON.stringify(config), "utf-8");
}

const validConfig = {
  network: { allowedDomains: ["github.com"] },
  filesystem: { denyRead: [denied], allowRead: [allowed], allowWrite: [allowed], denyWrite: [] },
};
...
function fakePi(options: { flags?: Record<string, boolean | string> } = {}) {
  ...
  return {
    ...
    tools,
    commands,
    call: (event: string, payload: unknown) => handlers.get(event)?.(payload, ctx),
...
function fakeRuntime(options: {
  ...
  const runtime = {
    ...
    async checkDependencies(): Promise<{ warnings: string[]; errors: string[] }> {
      return { warnings: options.depWarnings ?? [], errors: options.depErrors ?? [] };
    },
...
async function startSession(host: ReturnType<typeof fakePi>): Promise<void> {
  await host.call("session_start", { type: "session_start", reason: "startup" });
}

function readCall(path: string): ToolCallEvent {
  return { type: "tool_call", toolCallId: "call-1", toolName: "read", input: { path } } as ToolCallEvent;
}
...
test("fences the bash Tool and registers the guard's commands when the session starts", async () => {
  ...
  assert.ok(host.commands.has("guard"), "a status command should be registered");
  assert.ok(host.commands.has("guard-allow"), "a grant command should be registered");
...
test("blocks a Tool call the policy refuses", async () => {
  ...
  const decision = (await host.call("tool_call", readCall(join(denied, "file.txt")))) as {
    block?: boolean;
    reason?: string;
  };
...
test("fences user-run shell commands through the sandbox too", async () => {
  ...
  assert.ok(result.operations, "the user's own commands must run under the OS fence");
});

test("fails closed when the config cannot be loaded", async () => {
  writeConfig({ ...validConfig, filesystem: { ...validConfig.filesystem, allowWrites: ["."] } });
  const host = fakePi();
  guardExtension(host.pi, { runtime: fakeRuntime(), agentDir });
  await startSession(host);

  const decision = (await host.call("tool_call", readCall(join(allowed, "file.txt")))) as {
    block?: boolean;
    reason?: string;
  };
...
test("grantholders: /guard-allow admits a refused path for the session", async () => {
  ...
  assert.ok(((await host.call("tool_call", readCall(path))) as { block?: boolean }).block);
...
test("grantholders: /guard-allow tool:<name> admits an unmapped Tool", async () => {
  ...
  assert.ok(((await host.call("tool_call", call)) as { block?: boolean }).block);
...
test("refuses to start when a sandbox dependency is missing", async () => {
  ...
  assert.equal(runtime.initialized, 0, "a half-installed sandbox must not be initialized");
});

test("starts anyway when a dependency check only warns", async () => {
  writeConfig(validConfig);
  const host = fakePi();
  const runtime = fakeRuntime({ depWarnings: ["nested sandboxing will be weaker"] });
  guardExtension(host.pi, { runtime, agentDir });
...
test("registers the Windows command Tool so the adapter can fence it", async () => {
  writeConfig(validConfig);
  const host = fakePi();
  guardExtension(host.pi, { runtime: fakeRuntime(), agentDir, platform: "win32" });
...
test("registers only the shell Tools this platform has", async () => {
  writeConfig(validConfig);
  const host = fakePi();
  guardExtension(host.pi, { runtime: fakeRuntime(), agentDir, platform: "darwin" });
...
test("resets the runtime when the session ends", async () => {
  ...
  await host.call("session_shutdown", { type: "session_shutdown", reason: "quit" });
...
test("stays out of the way when --no-guard is passed", async () => {
  ...
  assert.deepEqual(host.tools, [], "no sandboxed bash tool should be registered");
  assert.equal(await host.call("tool_call", readCall(join(denied, "file.txt"))), undefined);
});
